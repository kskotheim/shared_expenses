...
class UserModifier {
  ...
  UserModifier({@required this.userId, @required this.shares, this.modifierId, this.fromDate, this.toDate, this.categories})
      : assert(userId != null),
        assert(shares != null);
  ...
  UserModifier.fromDocumentSnapshot(DocumentSnapshot userModifier) :
    ...
    fromDate = userModifier.data.containsKey(FROM) ? parseTime(userModifier.data[FROM]) : null,
    toDate = userModifier.data.containsKey(TO) ? parseTime(userModifier.data[TO]) : null,
    categories = userModifier.data.containsKey(CATEGORIES) ? List<String>.from(userModifier.data[CATEGORIES]) : null;
